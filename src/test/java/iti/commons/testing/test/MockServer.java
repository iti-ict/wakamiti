 ...
 */
package iti.commons.testing.test;
...
public class MockServer implements HttpHandler {
    ...
    public MockServer (Format format, String charset, int port) throws IOException {
    ...
    public MockServer(Format format, String charset, int port, String filename) throws IOException {
        configure(format,charset,port);
    ...
    protected void configure(Format format, String charset, int port) throws IOException {
    ...
    public void loadFromInputStream (InputStream stream) throws IOException {
        ...
        Map<String,Object> data = (Map<String,Object>) parse(stream);
    ...
    private Deque<Object> resolvePath(String path) throws IllegalAccessException {
    ...
    private void sendResponse (int status, Object value, HttpExchange exchange) throws IOException {
        ...
        exchange.getResponseHeaders().add("Content-Type", "application/"+format.toString().toLowerCase()+"; charset="+charset.toLowerCase());
    ...
    private Object parse(InputStream stream) throws IOException {
        if (format == Format.JSON) {
            return new JsonSlurper().parse(stream);
        }
        else if (format == Format.XML) {
            return new XStream().fromXML(stream);
        } else {
    ...
    private byte[] serialize(Object entity) throws IOException {
        if (entity == null) {
            return null;
        }
        if (format == Format.JSON) {
            ...
            return xstream.toXML(entity).getBytes(charset);
        } else {
            throw new IllegalArgumentException("format not supported: "+format);
        }
    }
