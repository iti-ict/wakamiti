...
public class TestKukumoNumberTypeEn {

    private static final Locale LOCALE = Locale.ENGLISH;

    @Test
    public void testAttempParseWithWrongValue() {
        final KukumoNumberDataType<Integer> type = KukumoNumberDataType.createFromNumber("int", Integer.class, false, Number::intValue);
        try {
            ...
            assertThat(e.getMessage()).startsWith("Error parsing type int using language en: 'xxxxx'");
        }
    }
    ...
    public void testBigDecimal() {
        final KukumoNumberDataType<BigDecimal> type = KukumoNumberDataType.createFromBigDecimal("bigdecimal", BigDecimal.class, true, x->x);
        ...
        assertThat(type.parse(LOCALE, "12,345.0")).isEqualByComparingTo(BigDecimal.valueOf(12345.0));
        assertThat(type.matcher(LOCALE, "12,345.54").matches()).isTrue();
        assertThat(type.parse(LOCALE, "12,345.54")).isEqualByComparingTo(BigDecimal.valueOf(12345.54));
        assertThat(type.matcher(LOCALE, "xxxxx").matches()).isFalse();
    }
