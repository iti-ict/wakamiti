...
public class TestTokenParser {
    ...
    private static final List<String> TOKENS = Arrays.asList(
            "mmmm","mm", "dd", "d", "yyyy", "yy","-", "\\", "/", " ");

    @Test
    public void testTokenParser1() {
        TokenParser parser = new TokenParser("mmmmmm/ ddd-yyyy-yy", TOKENS, Arrays.asList("'[^']*'"));
    ...
    public void testTokenParser2() {
        TokenParser parser = new TokenParser("d' de 'mmmm' de 'yyyy", TOKENS, Arrays.asList("'[^']*'"));
        assertNextToken(parser,"d");
