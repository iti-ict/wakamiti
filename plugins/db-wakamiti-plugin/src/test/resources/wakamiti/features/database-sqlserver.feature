...
Feature: Testing database steps
  ...
  Scenario: Test 1
    ...
    * On completion, the user satisfying the following SQL clause exists in the table client:
      """
      BIRTH_DATE < CAST('2000-01-01' as DATE)
      """
  ...
  Scenario: Test 2
    ...
    * On completion, the number of users satisfying the following in the table client is 0:
      | id | first_name | second_name | active | birth_date | creation                |
      | 1  | John       | Smith       | 1      | 2000-10-30 | 2024-07-22              |
    When the SQL script file '${data.dir}/db/dml-sqlserver.sql' is executed
    And the SQL procedure file '${data.dir}/db/procedure-sqlserver.sql' is executed
  ...
  Scenario: Test 3
    ...
    * On completion, the number of users satisfying the following in the table client is 0 using 'db' connection:
    ...
    And the user satisfying the following SQL clause exists in the table client in 1 second:
      """
      BIRTH_DATE = CAST('1980-12-25' as DATE)
      """
  ...
  Scenario: Schema-qualified table
    * On completion, the following SQL script is executed:
      ...
      INSERT INTO QUALIFIED.QUALIFIED_CLIENT (id, active) VALUES (1, 1);
      DELETE FROM other;
    ...
    When the following SQL script is executed:
      """
      UPDATE QUALIFIED.QUALIFIED_CLIENT SET active = 0 WHERE id = 1;
      """
    And the following SQL script is executed:
      """
      DELETE FROM other;
      INSERT INTO other (something)
      SELECT CASE WHEN active = 0 THEN 48 ELSE 47 END
