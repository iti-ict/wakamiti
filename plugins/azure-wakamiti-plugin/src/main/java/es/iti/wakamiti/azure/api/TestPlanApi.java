...
public class TestPlanApi extends BaseApi<TestPlanApi> {
    ...
    public TestPlanApi(URL baseUrl, Function<String, String> tagExtractor) {
        super(baseUrl);
    ...
    public Settings settings() {
        ...
        try {
            newRequest()
                    ...
                    .thenAccept(response -> response.body().ifPresent(json -> {
            ...
            LOGGER.warn("The zone id could not be obtained. The default is used: {}", ZoneId.systemDefault());
    ...
    public Optional<TestPlan> getTestPlan(String id) {
        String path = project() + "/testplan/plans/{planId}";
        return newRequest()
                .pathParam("planId", id)
                .get(path).body()
    ...
    public Optional<TestPlan> searchTestPlan(TestPlan plan) {
        Query query = new WorkItemsQuery()
                .select().where(field(teamProject()).isEqualsTo("@project")
        ...
        return newRequest()
                ...
                .map(json -> {
                    ...
                    return new TestCase().id(item.id()).name(item.name()).suite(suite)
                            ...
                            .testPointId(read(json, "$.pointAssignments[0].id", new TypeRef<>(){}));
                });
    }
    ...
    public List<TestSuite> createTestSuites(TestPlan plan, List<TestSuite> suites) {
        ...
        return Util.hasChildren(result);
    }
