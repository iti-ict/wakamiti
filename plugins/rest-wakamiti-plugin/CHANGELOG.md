...
### Added
- New step `rest.define.fromContentType` that sets the request content type using a MIME type string 
 (e.g. `application/json`), complementing the existing `rest.define.contentType` step that only accepts predefined 
 keyword values (e.g. `JSON`, `XML`).
...
### Fixed
- Key definition of the new steps.


## [2.9.2] - 2025-11-11

### Fixed
- Missing definition of the new steps.
