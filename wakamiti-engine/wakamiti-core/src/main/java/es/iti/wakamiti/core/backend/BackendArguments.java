...
public class BackendArguments implements Iterable<Pair<String, String>> {
    ...
    /*
     * If the method annotated with @Step accepts arguments and the field 'args'
        ...
        if (argumentMap.isEmpty()) {
            for (Class<?> methodArgType : methodArgTypes) {
                ...
                if (argType.isPresent()) {
                    argumentMap.add(new Pair<>(RunnableBackend.UNNAMED_ARG, argType.get().getName()));
                }
            }
        }
    }
    ...
    /*
     * Fill the argument map with the arguments that are not part of the step
     * definition (currently document string and datatables, but new extra
